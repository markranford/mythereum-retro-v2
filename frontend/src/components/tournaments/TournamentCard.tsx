...
import { Trophy, Users, Coins, Calendar, Bot } from 'lucide-react';
...
export default function TournamentCard({ tournament, onSelect }: TournamentCardProps) {
  ...
  const aiCount = tournament.participants.filter(p => p.isAI).length;
  const playerCount = tournament.participants.filter(p => !p.isAI).length;
    ...
    >
      ...
      <CardContent className="space-y-3">
          ...
          <div className="flex items-center gap-3 text-xs text-amber-200/50">
            ...
            {aiCount > 0 && (
              <span className="flex items-center gap-1">
                <Bot className="w-3 h-3 text-purple-400" /> {aiCount} AI
              </span>
            )}
          </div>
        )}
        ...
        <div className="flex items-center gap-2 text-amber-300/70 text-xs">
          ...
          {tournament.currentRound > 0 && <span>• Round {tournament.currentRound}</span>}
        </div>
        ...
        {champion && (
          <div className="pt-2 border-t border-amber-600/20">
            <div className="text-xs text-amber-300/70 flex items-center gap-1">
              ...
              Champion: <span className="text-amber-300 font-semibold">{champion.name}</span>
            </div>
          </div>
        )}

        {!champion && tournament.rewards && (
          <div className="pt-2 border-t border-amber-600/20">
            <div className="text-xs text-amber-300/70 mb-1">1st Place Reward</div>
